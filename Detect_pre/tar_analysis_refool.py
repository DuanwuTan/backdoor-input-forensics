...
MODEL_PATH = './record/refool_attack_2/attack_result.pt'
...
if p < 0.05:
    print('结论：Refool 后门样本注意力分布与干净样本存在显著差异 ✓')
else:
    print('结论：无显著差异')
...
axes[0].set_title(f'Refool TAR (p={p:.4f})')
axes[0].legend()
axes[1].boxplot([bd_entropies, clean_entropies], tick_labels=['Backdoor', 'Clean'])
axes[1].set_ylabel('Entropy')
axes[1].set_title('Refool Entropy Comparison')
plt.tight_layout()
plt.savefig('figures/0225/tar_analysis_refool.png', dpi=150)
print('图片已保存到 figures/0225/tar_analysis_refool.png')
